...
- **PR は必須にするが、承認（Approve）件数は 0** とする（`required_approving_review_count: 0`）。本プロジェクトのレビューは Code Reviewer / Reality Checker エージェントの所見を PR コメントとして記録する運用であり、GitHub の承認は用いないため（CLAUDE.md 参照）。承認必須（1件以上）にするとエージェント運用ではマージ不能になる。
...
- `required_pull_request_reviews.required_approving_review_count=0`：**PR 経由必須**（直 push を拒否）だが、**承認は0件でよい**。本プロジェクトのレビューは Code Reviewer / Reality Checker エージェントの所見を PR コメントに記録する運用であり、GitHub の承認（Approve）を課さないため。`required_pull_request_reviews` を `null` にすると「PR 必須」自体が無効になり直 push を防げない点に注意。
