...
`uv` をシステムに導入する（`command -v uv` で確認。無ければ
[公式手順](https://docs.astral.sh/uv/getting-started/installation/) で導入）。
`deploy/luno.service` は `ExecStart` をログインシェル経由（`/bin/bash -lc 'exec uv run …'`）に
しており、`uv` の導入先（`~/.local/bin` / `~/.cargo/bin` 等）の差を PATH 解決で吸収する。
ログイン PATH に `uv` が乗らない環境では、その行を絶対パス（`command -v uv` の値）に置き換える。
